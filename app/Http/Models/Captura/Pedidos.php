	...
	public function oracleCollections($collections) {
		...
		foreach ($collections->cursor() as $item) {
			foreach ($item->detalle as $key => $detalle) {
				...
				$detalle->load('cuadro_producto');

				if ($detalle->cuadro_producto) {
					...
					$po_lines = [
						...
						'ITEM_NUMBER' => $detalle->cuadro_producto->clave_cbn, # Artículo: Código del Artículo dado de alta en Inventarios y asignado a la organizacion dónde se realiza la Orden de compra. (Revisar LOV2)
						...
						'UNIT_MEAS_LOOKUP_CODE_T' => $detalle->cuadro_producto->unidad_medida2 ?? 'ENVASE', # UDM: Unidad de medida asignada al artículo en el módulo de inventarios. (Dependiente LOV2)
